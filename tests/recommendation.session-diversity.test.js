...
test('후속 조건 질의에서 후보가 충분하면 직전 main 반복을 우선 배제한다', async () => {
  const products = [
  ...
  const sessionKey = 'test-diversity-fresh-only';
  ...
  assert.equal(overlapCount(firstTop2, secondTop2), 0);
  assert.equal(
...
test('후속 조건 질의에서 신규 후보가 부족하면 직전 main 재사용을 허용한다', async () => {
  const products = [
